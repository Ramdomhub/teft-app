...
const RANKS: RankInfo[] = [
  ...
  { name: "SOLDIER",           color: "#94A3B8", minTeft: 10_000,    minLegion: 0  },
  { name: "SENTINEL",          color: "#60A5FA", minTeft: 100_000,   minLegion: 0  },
  { name: "VETERAN",           color: "#818CF8", minTeft: 500_000,   minLegion: 0  },
  { name: "COMMANDER",         color: "#A78BFA", minTeft: 0,         minLegion: 5  },
  { name: "PHANTOM",           color: "#C084FC", minTeft: 1_000_000, minLegion: 0  },
  { name: "PHANTOM COMMANDER", color: "#E879F9", minTeft: 1_000_000, minLegion: 10 },
  { name: "LEGION ELITE",      color: "#F0ABFC", minTeft: 5_000_000, minLegion: 10 },
];
...
function getBadges(teft: number, legion: number, joinedAt: string, xVerified: boolean): Badge[] {
  ...
  if (teft >= 1_000_000)   badges.push({ id: "whale",     label: "Whale",            emoji: "🐋", desc: "Holds 1M+ TEFT" });
...
function WalletAvatar({ address, size = 64 }: { address: string; size?: number }) {
  ...
  return (
    <svg width={s} height={s} viewBox={`0 0 ${s} ${s}`} style={{ borderRadius: "50%", display: "block" }}>
      <defs><clipPath id={`cl-${address.slice(0,6)}`}><circle cx={s/2} cy={s/2} r={s/2}/></clipPath></defs>
      <g clipPath={`url(#cl-${address.slice(0,6)})`}>
        <rect width={s} height={s} fill="#0a0a0a"/>
        {cells.map((v, i) => <rect key={i} x={(i%3)*cell} y={Math.floor(i/3)*cell} width={cell} height={cell} fill={cellColors[v]} opacity={0.9}/>)}
...
type CardData = { wallet: string; teft: number; legion: number; joinedAt: string; referredBy: string|null; xHandle: string|null; xVerified: boolean };
    ...
    <div style={{ position:"relative", background:"linear-gradient(160deg,#0e0e0e 0%,#141414 60%,#0a0a0a 100%)", border:`1px solid ${rank.color}33`, borderRadius:20, padding:24, overflow:"hidden", boxShadow:`0 0 40px ${rank.color}15,inset 0 1px 0 ${rank.color}18`, fontFamily:"'DM Mono','Fira Mono',monospace", width:"100%" }}>
      ...
      <div style={{ marginBottom:16 }}>
        ...
        <div style={{ height:2, background:"#1a1a1a", borderRadius:2, overflow:"hidden" }}>
          <div style={{ height:"100%", width:`${Math.min(100,(data.teft/5_000_000)*100)}%`, background:`linear-gradient(90deg,${rank.color}66,${rank.color})`, borderRadius:2 }}/>
        </div>
      </div>
...
function TokenGate({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const doSign = signMessage ? signMessage(msg).then(() => true).catch(() => false) : Promise.resolve(true);
    doSign.then(ok => {
  ...
  const box: React.CSSProperties = { minHeight:"100vh", background:"#080808", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:16, fontFamily:"'DM Mono',monospace" };
  ...
  if (!connected) return <div style={box}><style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style><div style={{fontSize:40}}>🪪</div><div style={{color:"#fff",fontSize:18,fontWeight:800}}>TEFT Identity</div><div style={{color:"#444",fontSize:12}}>Connect wallet to access your Identity</div><WalletMultiButton style={{marginTop:8}}/></div>;
  if (state==="signing")  return <div style={box}><style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style>{spinner}<div style={{color:"#555",fontSize:12}}>Sign to verify…</div></div>;
  if (state==="checking") return <div style={box}><style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style>{spinner}<div style={{color:"#555",fontSize:12}}>Checking TEFT balance…</div></div>;
  if (state==="error")    return <div style={box}><div style={{color:"#F87171",fontSize:13}}>Signing cancelled.</div><button onClick={()=>{signed.current=false;setState("idle");}} style={{background:"#111",color:"#fff",border:"1px solid #333",borderRadius:8,padding:"10px 20px",cursor:"pointer",fontSize:12}}>Try Again</button></div>;
  if (state==="denied")   return <div style={box}><div style={{fontSize:44}}>🚫</div><div style={{color:"#fff",fontSize:18,fontWeight:800}}>No Access</div><div style={{color:"#555",fontSize:12,textAlign:"center",maxWidth:260}}>You need at least <strong style={{color:"#fff"}}>1 TEFT</strong> to enter.</div><a href={`https://jup.ag/swap/SOL-${TEFT_MINT}?referral=${JUPITER_REF}&feeBps=50`} target="_blank" rel="noreferrer" style={{background:"#4ADE80",color:"#000",borderRadius:10,padding:"12px 28px",fontWeight:800,fontSize:13,textDecoration:"none",marginTop:8}}>Buy TEFT on Jupiter ↗</a></div>;
  return <>{children}</>;
}
          ...
          {!loading && cardData && (
            <>
              ...
              <div style={{ marginTop:20, background:"#0d0d0d", border:"1px solid #1a1a1a", borderRadius:14, padding:18 }}>
                ...
                {cardData.legion > 0 && <div style={{ marginTop:10, fontSize:11, color:"#4ADE80" }}>👥 {cardData.legion} soldier{cardData.legion!==1?"s":""} in your Legion</div>}
              </div>
...
ENDOFFILEmkdir -p ~/Desktop/teft-app/app/identity && cat > ~/Desktop/teft-app/app/identity/page.tsx << 'ENDOFFILE'
"use client";
