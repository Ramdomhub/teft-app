  ...
  return (
    <div className="min-h-screen bg-[#f5f5f7] text-[#1d1d1f] font-sans antialiased pb-20 selection:bg-black selection:text-white">
      ...
      <section className="pt-48 pb-20 px-4 text-center">
        <h1 className="text-7xl md:text-9xl font-[900] tracking-tighter mb-8 leading-[0.85] uppercase">
          LIVE<br/><span className="text-zinc-200">ACTIVITY.</span>
        </h1>
      ...
      <section className="max-w-3xl mx-auto px-4 mb-24 grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white p-12 rounded-[3.5rem] shadow-sm border border-black/[0.01]">
          <p className="text-[10px] font-black uppercase tracking-[0.4em] text-zinc-300 mb-4">Total Support</p>
          <p className="text-5xl font-[900] tracking-tighter">{stats.totalVolume.toFixed(1).replace('.', ',')} PTS</p>
        </div>
        <div className="bg-white p-12 rounded-[3.5rem] shadow-sm border border-black/[0.01]">
          <p className="text-[10px] font-black uppercase tracking-[0.4em] text-zinc-300 mb-4">Verified Creators</p>
      ...
      <section className="max-w-xl mx-auto px-4">
        ...
        <div className="space-y-3">
          {recentTips.length > 0 ? recentTips.map((tip) => (
            ...
            <p className="text-center text-[10px] font-black uppercase text-zinc-300 tracking-widest">No recent transmissions</p>
          )}
        </div>
      </section>
    </div>
